...
func (s *PrecompileTestSuite) TestWithdrawDelegatorRewards() {
	...
	for _, tc := range testCases {
		s.Run(tc.name, func() {
			...
			// sanity check to make sure the starting balance is always 5 QUBETICS
			balance := s.app.BankKeeper.GetBalance(s.ctx, s.address.Bytes(), utils.BaseDenom)
...
func (s *PrecompileTestSuite) TestClaimRewards() {
	...
	for _, tc := range testCases {
		s.Run(tc.name, func() {
			...
			// Distribute rewards to the 2 validators, 1 QUBETICS each
...
func (s *PrecompileTestSuite) TestFundCommunityPool() {
	...
	testCases := []struct {
		...
		{
			"success - fund the community pool 1 QUBETICS",
			func() []interface{} {
				return []interface{}{
					s.address,
					big.NewInt(1e18),
