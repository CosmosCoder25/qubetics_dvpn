...
import (
	...
	auth "github.com/qubetics/qubetics-blockchain/v2/precompiles/authorization"
	"github.com/qubetics/qubetics-blockchain/v2/precompiles/erc20"
	"github.com/qubetics/qubetics-blockchain/v2/precompiles/erc20/testdata"
	"github.com/qubetics/qubetics-blockchain/v2/precompiles/testutil"
	contractutils "github.com/qubetics/qubetics-blockchain/v2/precompiles/testutil/contracts"
...
func (is *IntegrationTestSuite) SetupTest() {
	...
	// Set up min deposit in Qubetics
...
var (
	...
	wqubeticsAddress   common.Address
...
var _ = Describe("ERC20 Extension -", func() {
	...
	BeforeEach(func() {
		...
		wqubeticsAddress = common.HexToAddress(erc20Params.NativePrecompiles[0])
	...
	Context("basic functionality -", func() {
		...
		When("calling reverter contract", func() {
			Context("in a direct call to the WQUBETICS contract", func() {
				...
				DescribeTable("it should revert token transfer from the WQUBETICS contract", func(before bool, after bool) {
					sender := is.keyring.GetKey(0)
					receiver := is.keyring.GetAddr(1)
					amountToSend := big.NewInt(100)
					denomInitialBalance := is.network.App.BankKeeper.GetBalance(is.network.GetContext(), receiver.Bytes(), is.bondDenom)
				...
				It("it should send token transfer and send from WQUBETICS contract", func() {
					sender := is.keyring.GetKey(0)
					receiver := is.keyring.GetAddr(1)
				...
				DescribeTable("it should revert token transfer and send from WQUBETICS contract", func(before bool, after bool) {
					sender := is.keyring.GetKey(0)
					receiver := is.keyring.GetAddr(1)
					denomInitialBalance := is.network.App.BankKeeper.GetBalance(is.network.GetContext(), receiver.Bytes(), is.bondDenom)
					senderInitialBalance := is.network.App.BankKeeper.GetBalance(is.network.GetContext(), sender.AccAddr, is.bondDenom)
		...
		When("approving an allowance", func() {
			Context("in a call to the token contract", func() {
				...
				When("the grantee is the same as the granter", func() {
					...
					// changes to the Cosmos SDK.
					//
					// For reference see this comment: https://github.com/qubetics/qubetics/pull/2088#discussion_r1407646217
...
var _ = Describe("ERC20 Extension migration Flows -", func() {
	...
	When("using Qubetics (not wQubetics) in smart contracts", func() {
		It("should be using straight Qubetics for sending funds in smart contracts", func() {
			Skip("will be addressed in follow-up PR")
