...
import (
	...
	"github.com/qubetics/qubetics-blockchain/v2/precompiles/vesting/testdata"
...
func (s *PrecompileTestSuite) CreateTestClawbackVestingAccount(funder, vestingAddr common.Address) {
	...
	s.Require().NoError(err)
	err = qubeticsutil.FundAccount(s.ctx, s.app.BankKeeper, vestingAddr.Bytes(), sdk.NewCoins(sdk.NewCoin(utils.BaseDenom, math.NewInt(100))))
	s.Require().NoError(err)
...
func (s *PrecompileTestSuite) NextBlock() {
	var err error
	s.ctx, err = qubeticsutil.CommitAndCreateNewCtx(s.ctx, s.app, time.Second, nil)
