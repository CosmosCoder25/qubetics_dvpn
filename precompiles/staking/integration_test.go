...
import (
	...
	compiledcontracts "github.com/qubetics/qubetics-blockchain/v2/contracts"
	...
	"github.com/qubetics/qubetics-blockchain/v2/precompiles/staking"
	"github.com/qubetics/qubetics-blockchain/v2/precompiles/staking/testdata"
	"github.com/qubetics/qubetics-blockchain/v2/precompiles/testutil"
	"github.com/qubetics/qubetics-blockchain/v2/precompiles/testutil/contracts"
...
var _ = Describe("Calling staking precompile directly", func() {
	...
	Describe("Execute approve transaction", func() {
		...
		It("should approve the undelegate method with 1 qubetics", func() {
			s.SetupApproval(
		...
		It("should approve the redelegate method with 2 qubetics", func() {
			s.SetupApproval(
		...
		It("should approve the cancel unbonding delegation method with 1 qubetics", func() {
			s.SetupApproval(
	...
	Describe("Execute increase allowance transaction", func() {
		...
		It("Should increase the allowance of the delegate method with 1 qubetics", func() {
			increaseArgs := defaultCallArgs.
				WithMethodName(authorization.IncreaseAllowanceMethod).
				WithArgs(
	...
	Describe("Execute decrease allowance transaction", func() {
		...
		It("Should decrease the allowance of the delegate method with 1 qubetics", func() {
			decreaseArgs := defaultDecreaseArgs.WithArgs(
				s.precompile.Address(), big.NewInt(1e18), []string{staking.DelegateMsg},
			)
	...
	Describe("to revoke an approval", func() {
		...
		It("should not revoke the approval if sent by someone else than the granter", func() {
			...
			err := qubeticsutil.FundAccountWithBaseDenom(s.ctx, s.app.BankKeeper, differentAddr.Bytes(), 1e18)
	...
	Describe("to edit validator", func() {
		...
		Context("when origin is equal to validator address", func() {
			It("should succeed", func() {
				...
				err := qubeticsutil.FundAccountWithBaseDenom(s.ctx, s.app.BankKeeper, newAddr, 2e18)
	...
	Describe("Calling precompile txs from a vesting account", func() {
		...
		BeforeEach(func() {
			...
			vestingAmtTotal := qubeticsutil.TestVestingSchedule.TotalVestingCoins
		...
		Context("before first vesting period - all tokens locked and unvested", func() {
			BeforeEach(func() {
				...
				Expect(unvested).To(Equal(qubeticsutil.TestVestingSchedule.TotalVestingCoins), "expected different unvested coins")
			...
			It("Should be able to delegate tokens not involved in vesting schedule", func() {
				...
				err := qubeticsutil.FundAccount(s.ctx, s.app.BankKeeper, clawbackAccount.GetAddress(), coinsToDelegate)
				Expect(err).To(BeNil())

				// check balance is updated
				balance := s.app.BankKeeper.GetBalance(s.ctx, clawbackAccount.GetAddress(), s.bondDenom)
				Expect(balance).To(Equal(accountGasCoverage[0].Add(qubeticsutil.TestVestingSchedule.TotalVestingCoins[0]).Add(coinsToDelegate[0])))
				...
				Expect(balancePost.IsGTE(qubeticsutil.TestVestingSchedule.TotalVestingCoins[0])).To(BeTrue())
			})
		})
		...
		Context("after first vesting period and before lockup - some vested tokens, but still all locked", func() {
			BeforeEach(func() {
				...
				cliffDuration := time.Duration(qubeticsutil.TestVestingSchedule.CliffPeriodLength)
				s.NextBlockAfter(cliffDuration * time.Second)

				// Check if some, but not all tokens are vested
				vested = clawbackAccount.GetVestedCoins(s.ctx.BlockTime())
				expVested := sdk.NewCoins(sdk.NewCoin(s.bondDenom, qubeticsutil.TestVestingSchedule.VestedCoinsPerPeriod[0].Amount.Mul(math.NewInt(qubeticsutil.TestVestingSchedule.CliffMonths))))
				Expect(vested).NotTo(Equal(qubeticsutil.TestVestingSchedule.TotalVestingCoins), "expected some tokens to have been vested")
			...
			It("Should be able to delegate locked vested tokens + free tokens (not in vesting schedule)", func() {
				...
				err := qubeticsutil.FundAccount(s.ctx, s.app.BankKeeper, clawbackAccount.GetAddress(), amt)
				Expect(err).To(BeNil())

				// check balance is updated
				balance := s.app.BankKeeper.GetBalance(s.ctx, clawbackAccount.GetAddress(), s.bondDenom)
				Expect(balance).To(Equal(accountGasCoverage[0].Add(qubeticsutil.TestVestingSchedule.TotalVestingCoins[0]).Add(amt[0])))
		...
		Context("Between first and second lockup periods - vested coins are unlocked", func() {
			BeforeEach(func() {
				...
				vestDuration := time.Duration(qubeticsutil.TestVestingSchedule.LockupPeriodLength)
				...
				expVested := sdk.NewCoins(sdk.NewCoin(s.bondDenom, qubeticsutil.TestVestingSchedule.VestedCoinsPerPeriod[0].Amount.Mul(math.NewInt(qubeticsutil.TestVestingSchedule.LockupMonths))))
				expUnlockedVested := expVested

				Expect(vested).NotTo(Equal(qubeticsutil.TestVestingSchedule.TotalVestingCoins), "expected not all tokens to be vested")
				...
				Expect(unlocked).To(Equal(qubeticsutil.TestVestingSchedule.UnlockedCoinsPerLockup))
	...
	Describe("Redelegations queries", func() {
		...
		It("should return an empty array if no redelegation is found for the given source validator", func() {
			...
			// possible (see https://github.com/qubetics/cosmos-sdk/blob/e773cf768844c87245d0c737cda1893a2819dd89/x/staking/keeper/querier.go#L361-L373):
			//
...
var _ = Describe("Calling staking precompile via Solidity", func() {
	...
	BeforeEach(func() {
		...
		err = qubeticsutil.FundAccount(s.ctx, s.app.BankKeeper, contractTwoAddr.Bytes(), sdk.NewCoins(sdk.NewCoin(utils.BaseDenom, testContractInitialBalance)))
		Expect(err).To(BeNil(), "error while funding the smart contract: %v", err)

		err = qubeticsutil.FundAccount(s.ctx, s.app.BankKeeper, stkReverterAddr.Bytes(), sdk.NewCoins(sdk.NewCoin(utils.BaseDenom, testContractInitialBalance)))
		Expect(err).To(BeNil(), "error while funding the smart contract: %v", err)
	...
	Context("create a validator", func() {
		...
		BeforeEach(func() {
			...
			err := qubeticsutil.FundAccountWithBaseDenom(s.ctx, s.app.BankKeeper, valAddr, 1e18)
			Expect(err).To(BeNil(), "error while funding account: %v", err)
	...
	Context("to edit a validator", func() {
		...
		BeforeEach(func() {
			...
			err := qubeticsutil.FundAccountWithBaseDenom(s.ctx, s.app.BankKeeper, valAddr, 2e18)
			Expect(err).To(BeNil(), "error while funding account: %v", err)
	...
	Context("delegating", func() {
		...
		Context("with approval set", func() {
			...
			It("should not delegate when sending from a different address", func() {
				newAddr, newPriv := testutiltx.NewAccAddressAndKey()
				err := qubeticsutil.FundAccountWithBaseDenom(s.ctx, s.app.BankKeeper, newAddr, 1e18)
				Expect(err).To(BeNil(), "error while funding account: %v", err)
		...
		Describe("delegation from a vesting account", func() {
			...
			BeforeEach(func() {
				...
				// Check if all tokens are unvested at vestingStart
				totalVestingCoins := qubeticsutil.TestVestingSchedule.TotalVestingCoins
				unvested = clawbackAccount.GetVestingCoins(s.ctx.BlockTime())
				vested = clawbackAccount.GetVestedCoins(s.ctx.BlockTime())
			...
			Context("before first vesting period - all tokens locked and unvested", func() {
				BeforeEach(func() {
					...
					Expect(unvested).To(Equal(qubeticsutil.TestVestingSchedule.TotalVestingCoins))
			...
			Context("after first vesting period and before lockup - some vested tokens, but still all locked", func() {
				BeforeEach(func() {
					...
					vested = clawbackAccount.GetVestedCoins(s.ctx.BlockTime())
					expVested := sdk.NewCoins(sdk.NewCoin(s.bondDenom, qubeticsutil.TestVestingSchedule.VestedCoinsPerPeriod[0].Amount.Mul(math.NewInt(qubeticsutil.TestVestingSchedule.CliffMonths))))
					Expect(vested).NotTo(Equal(qubeticsutil.TestVestingSchedule.TotalVestingCoins))
...
var _ = Describe("Batching cosmos and eth interactions", func() {
	...
	BeforeEach(func() {
		...
		contractAddr, err = qubeticsutil.DeployContract(s.ctx, s.app, s.privKey, s.queryClientEVM, stakingCallerContract)
		Expect(err).To(BeNil(), "error while deploying the StakingCaller contract")

		// Deploy ERC20 contract
		erc20ContractAddr, err = qubeticsutil.DeployContract(s.ctx, s.app, s.privKey, s.queryClientEVM, erc20Contract,
