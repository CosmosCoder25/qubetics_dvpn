...
import (
	...
	qubeticscontracts "github.com/qubetics/qubetics-blockchain/v2/contracts"
	...
	"github.com/qubetics/qubetics-blockchain/v2/precompiles/erc20"
...
// SetupWithGenesisValSet initializes a new QubeticsApp with a validator set and genesis accounts
...
func (s *PrecompileTestSuite) SetupWithGenesisValSet(valSet *tmtypes.ValidatorSet, genAccs []authtypes.GenesisAccount, balances ...banktypes.Balance) {
	appI, genesisState := qubeticsapp.SetupTestingApp(cmn.DefaultChainID)()
	app, ok := appI.(*qubeticsapp.Qubetics)
	s.Require().True(ok)
	...
	bondAmt := sdk.TokensFromConsensusPower(1, qubeticstypes.PowerReduction)
	...
	app.InitChain(
		abci.RequestInitChain{
			...
			ConsensusParams: qubeticsapp.DefaultConsensusParams,
...
func (s *PrecompileTestSuite) NewTestChainWithValSet(coord *ibctesting.Coordinator, valSet *tmtypes.ValidatorSet, signers map[string]tmtypes.PrivValidator) *ibctesting.TestChain {
	...
	addr, priv := qubeticsutiltx.NewAddrKey()
	s.privKey = priv
	s.address = addr
	// differentAddr is an address generated for testing purposes that e.g. raises the different origin error
	s.differentAddr = qubeticsutiltx.GenerateAddress()
	s.signer = qubeticsutiltx.NewSigner(priv)

	baseAcc := authtypes.NewBaseAccount(priv.PubKey().Address().Bytes(), priv.PubKey(), 0, 0)

	acc := &qubeticstypes.EthAccount{
		BaseAccount: baseAcc,
		CodeHash:    common.BytesToHash(evmtypes.EmptyCodeHash).Hex(),
	}

	amount := sdk.TokensFromConsensusPower(5, qubeticstypes.PowerReduction)
...
func (s *PrecompileTestSuite) NewTransferAuthorizationWithAllocations(ctx sdk.Context, app *qubeticsapp.Qubetics, grantee, granter common.Address, allocations []transfertypes.Allocation) error {
...
func (s *PrecompileTestSuite) NewTransferAuthorization(ctx sdk.Context, app *qubeticsapp.Qubetics, grantee, granter common.Address, path *ibctesting.Path, coins sdk.Coins, allowList []string, allowedPacketData []string) error {
...
func (s *PrecompileTestSuite) setupIBCTest() {
	...
	s.app = s.chainA.App.(*qubeticsapp.Qubetics)
	...
	s.transferPath = qubeticsibc.NewTransferPath(s.chainA, s.chainB) // clientID, connectionID, channelID empty
	qubeticsibc.SetupPath(s.coordinator, s.transferPath)             // clientID, connectionID, channelID filled
...
// TODO upstream this change to qubetics (adding gasPrice)
// DeployContract deploys a contract with the provided private key,
// compiled contract data and constructor arguments
func DeployContract(
	ctx sdk.Context,
	qubeticsApp *qubeticsapp.Qubetics,
	...
	chainID := qubeticsApp.EvmKeeper.ChainID()
	from := common.BytesToAddress(priv.PubKey().Address().Bytes())
	nonce := qubeticsApp.EvmKeeper.GetNonce(ctx, from)
	...
	gas, err := qubeticsutiltx.GasLimit(ctx, from, data, queryClientEvm)
	if err != nil {
		return common.Address{}, err
	}
	...
	msgEthereumTx := evmtypes.NewTx(&evmtypes.EvmTxArgs{
		...
		Nonce:     nonce,
		GasLimit:  gas,
		GasFeeCap: qubeticsApp.FeeMarketKeeper.GetBaseFee(ctx),
	...
	res, err := qubeticsutil.DeliverEthTx(qubeticsApp, priv, msgEthereumTx)
	if err != nil {
		return common.Address{}, err
	}

	if _, err := qubeticsutil.CheckEthTxResponse(res, qubeticsApp.AppCodec()); err != nil {
		return common.Address{}, err
	}
...
func (s *PrecompileTestSuite) DeployERC20Contract(chain *ibctesting.TestChain, name, symbol string, decimals uint8) (common.Address, error) {
	addr, err := DeployContract(
		...
		gasPrice,
		s.queryClientEVM,
		qubeticscontracts.ERC20MinterBurnerDecimalsContract,
		name,
		symbol,
		decimals,
	)
...
func (s *PrecompileTestSuite) setupERC20ContractTests(amount *big.Int) common.Address {
	...
	defaultERC20CallArgs := contracts.CallArgs{
		...
		ContractABI:  qubeticscontracts.ERC20MinterBurnerDecimalsContract.ABI,
		PrivKey:      s.privKey,
		GasPrice:     gasPrice,
	}
	...
	mintCheck := testutil.LogCheckArgs{
		ABIEvents: qubeticscontracts.ERC20MinterBurnerDecimalsContract.ABI.Events,
	...
	balance := s.app.Erc20Keeper.BalanceOf(
		s.chainA.GetContext(),
		qubeticscontracts.ERC20MinterBurnerDecimalsContract.ABI,
		erc20Addr,
		s.address,
	)
