...
func TestGetQubeticsAddressFromBech32(t *testing.T) {
	testCases := []struct {
		name       string
		address    string
		expAddress string
		...
		{
			"qubetics address",
			"qubetics1qql8ag4cluz6r4dz28p3w00dnc9w8ueu8jcj58",
			"qubetics1qql8ag4cluz6r4dz28p3w00dnc9w8ueu8jcj58",
			false,
		},
		{
	...
	for _, tc := range testCases {
		addr, err := GetQubeticsAddressFromBech32(tc.address)
		if tc.expError {
...
func TestQubeticsCoinDenom(t *testing.T) {
	testCases := []struct {
		name     string
		denom    string
		expError bool
