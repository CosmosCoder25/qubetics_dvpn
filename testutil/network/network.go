...
func DefaultConfig() Config {
	...
	chainID := fmt.Sprintf("qubetics_%d-1", tmrand.Int63n(9999999999999)+1)
	return Config{
		Codec:             encCfg.Codec,
		...
		NumValidators:     4,
		BondDenom:         "tics",
		MinGasPrices:      fmt.Sprintf("0.000006%s", qubeticstypes.AttoQubetics),
		AccountTokens:     sdk.TokensFromConsensusPower(1000000000000000000, qubeticstypes.PowerReduction),
		StakingTokens:     sdk.TokensFromConsensusPower(500000000000000000, qubeticstypes.PowerReduction),
		BondedTokens:      sdk.TokensFromConsensusPower(100000000000000000, qubeticstypes.PowerReduction),
...
// NewAppConstructor returns a new Qubetics AppConstructor
...
func New(l Logger, baseDir string, cfg Config) (*Network, error) {
	...
	if !qubeticstypes.IsValidChainID(cfg.ChainID) {
	...
	for i := 0; i < cfg.NumValidators; i++ {
		...
		nodeDir := filepath.Join(network.BaseDir, nodeDirName, "qubeticsd")
		clientDir := filepath.Join(network.BaseDir, nodeDirName, "qubeticscli")
		...
		genAccounts = append(genAccounts, &qubeticstypes.EthAccount{
		...
		customAppTemplate, _ := config.AppConfig(qubeticstypes.AttoQubetics)
