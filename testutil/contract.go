...
type ContractCallArgs struct {
	...
	Nonce *big.Int
	// Amount is the tics amount to send in the transaction.
	Amount *big.Int
...
func DeployContract(
	...
	res, err := DeliverEthTx(qubeticsApp, priv, msgEthereumTx)
	if err != nil {
		return common.Address{}, err
	}

	if _, err := CheckEthTxResponse(res, qubeticsApp.AppCodec()); err != nil {
		return common.Address{}, err
	}
...
func DeployContractWithFactory(
	...
	chainID := qubeticsApp.EvmKeeper.ChainID()
	from := common.BytesToAddress(priv.PubKey().Address().Bytes())
	factoryNonce := qubeticsApp.EvmKeeper.GetNonce(ctx, factoryAddress)
	nonce := qubeticsApp.EvmKeeper.GetNonce(ctx, from)
