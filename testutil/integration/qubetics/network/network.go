...
import (
	...
	commonnetwork "github.com/qubetics/qubetics-blockchain/v2/testutil/integration/common/network"
...
type Network interface {
	...
	UpdateMintParams(params minttypes.Params) error
...
func (n *IntegrationNetwork) configureAndInitChain() error {
	...
	// Create a new QubeticsApp with the following params
	qubeticsApp := createQubeticsApp(n.cfg.chainID)
	...
	genesisState, err = customizeGenesis(qubeticsApp, n.cfg.customGenesisState, genesisState)
	if err != nil {
		return err
	}
	...
	header := tmproto.Header{
		ChainID:            n.cfg.chainID,
		Height:             qubeticsApp.LastBlockHeight() + 1,
		Time:               now,
		AppHash:            qubeticsApp.LastCommitID().Hash,
		ValidatorsHash:     valSet.Hash(),
		NextValidatorsHash: valSet.Hash(),
		ProposerAddress:    valSet.Proposer.Address,
	}
	qubeticsApp.BeginBlock(abcitypes.RequestBeginBlock{Header: header})

	// Set networks global parameters
	n.app = qubeticsApp
	// TODO - this might not be the best way to initialize the context
	n.ctx = qubeticsApp.BaseApp.NewContext(false, header)
	...
	// Register QUBETICS in denom metadata
	qubeticsMetadata := banktypes.Metadata{
		Description: "The native token of Qubetics",
		Base:        n.cfg.denom,
	...
	qubeticsApp.BankKeeper.SetDenomMetaData(n.ctx, qubeticsMetadata)
