...
var (
	...
	DefaultFee = sdk.NewCoin(utils.BaseDenom, sdkmath.NewIntFromUint64(uint64(feeAmt))) // 0.01 QUBETICS
)
...
type CosmosTxArgs struct {
	...
	// ChainID is the chain's id on cosmos format, e.g. 'qubetics_9000-1'
	ChainID string
...
func signCosmosTx(
	...
	seq, err := appQubetics.AccountKeeper.GetSequence(ctx, addr)
	if err != nil {
		return nil, err
	}
	...
	accNumber := appQubetics.AccountKeeper.GetAccount(ctx, addr).GetAccountNumber()
