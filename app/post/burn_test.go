...
func (s *PostTestSuite) TestPostHandle() {
	testCases := []struct {
		...
		},
		{
			...
			tx: func() sdk.Tx {
				feeAmount := sdk.Coins{sdk.Coin{Amount: sdkmath.NewInt(10), Denom: "tics"}}
				amount := sdk.Coins{sdk.Coin{Amount: sdkmath.NewInt(20), Denom: "tics"}}
				s.MintCoinsForFeeCollector(amount)
			...
			expPass: true,
			postChecks: func() {
				expected := sdk.Coins{sdk.Coin{Amount: sdkmath.NewInt(10), Denom: "tics"}}
				balance := s.GetFeeCollectorBalance()
				s.Require().Equal(expected, balance)
			},
		},
		{
			...
			tx: func() sdk.Tx {
				feeAmount := sdk.Coins{
					...
					sdk.Coin{Amount: sdkmath.NewInt(10), Denom: "tics"},
				}
				amount := feeAmount
				s.MintCoinsForFeeCollector(amount)
		...
		{
			name: "pass - fees exceeds MaxUint64 (~18 FU). Should not panic",
			tx: func() sdk.Tx {
				amt, ok := sdkmath.NewIntFromString("10000000000000000000000000000000000")
				s.Require().True(ok)
				feeAmount := sdk.Coins{sdk.Coin{Amount: amt, Denom: "tics"}}
				amount := sdk.Coins{sdk.Coin{Amount: amt, Denom: "tics"}}
				s.MintCoinsForFeeCollector(amount)
