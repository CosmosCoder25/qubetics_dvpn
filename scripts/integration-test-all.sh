...
## default port prefixes for qubeticsd
NODE_P2P_PORT="2660"
NODE_PORT="2663"
NODE_RPC_PORT="2666"
...
init_func() {
    "$PWD"/build/qubeticsd keys add $KEY"$i" --keyring-backend test --home "$DATA_DIR$i" --no-backup --algo "eth_secp256k1"
    "$PWD"/build/qubeticsd init $MONIKER --chain-id $CHAINID --home "$DATA_DIR$i"
    "$PWD"/build/qubeticsd add-genesis-account \
    "$("$PWD"/build/qubeticsd keys show "$KEY$i" --keyring-backend test -a --home "$DATA_DIR$i")" 1000000000000000000tics,1000000000000000000stake \
    --keyring-backend test --home "$DATA_DIR$i"
    "$PWD"/build/qubeticsd gentx "$KEY$i" 1000000000000000000stake --chain-id $CHAINID --keyring-backend test --home "$DATA_DIR$i"
    "$PWD"/build/qubeticsd collect-gentxs --home "$DATA_DIR$i"
    "$PWD"/build/qubeticsd validate-genesis --home "$DATA_DIR$i"
...
start_func() {
    ...
    "$PWD"/build/qubeticsd start --pruning=nothing --rpc.unsafe \
    ...
    QUBETICS_PID=$!
    echo "started qubetics node, pid=$QUBETICS_PID"
    # add PID to array
    arr+=("$QUBETICS_PID")
...
if [[ -z $TEST || $TEST == "rpc" ||  $TEST == "pending" ]]; then
    ...
    for i in $(seq 1 "$TEST_QTD"); do
        ...
        echo "going to test qubetics node $HOST_RPC ..."
...
stop_func() {
    QUBETICS_PID=$i
    echo "shutting down node, pid=$QUBETICS_PID ..."

    # Shutdown qubetics node
    kill -9 "$QUBETICS_PID"
    wait "$QUBETICS_PID"
