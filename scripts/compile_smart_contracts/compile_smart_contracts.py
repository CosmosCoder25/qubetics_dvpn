...
# The path to the main level of the Qubetics repository.
...
def is_qubetics_repo(path: Path) -> bool:
    """
    This function checks if the given path is the root of the Qubetics repository,
    ...
    with open(path / "go.mod", "r", encoding="utf-8") as go_mod:
        while True:
            ...
            if "module github.com/qubetics/qubetics" in line:
                return True
...
if __name__ == "__main__":
    if not is_qubetics_repo(REPO_PATH):
        raise ValueError(
            "This script should only be executed "
            + "in the qubetics repository."
