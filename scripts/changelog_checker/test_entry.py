...
class TestEntry:
    ...
    example = (
        "- (distribution-precompile) [#1949](https://github.com/qubetics/qubetics/pull/1949) "
    ...
    def test_pass_includes_link(self):
        example = (
            "- (evm) [#1851](https://github.com/qubetics/qubetics/pull/1851) "
            + "Enable [EIP 3855](https://eips.ethereum.org/EIPS/eip-3855) "
            + "(`PUSH0` opcode) during upgrade."
        )
        entry = Entry(example)
        ok = entry.parse()
        assert entry.link == "https://github.com/qubetics/qubetics/pull/1851"
    ...
    def test_fail_has_backslash_in_link(self):
        example = r"- (evm) [\#1851](https://github.com/qubetics/qubetics/pull/1851) Test."
        entry = Entry(example)
        ok = entry.parse()
    ...
    def test_entry_wrong_pr_link_and_missing_dot(self):
        entry = Entry(
            "- (distribution-precompile) [#1949](https://github.com/qubetics/qubetics/pull/1948) "
            + "Add `ClaimRewards` custom transaction"
        )
        assert entry.parse() is False
        assert entry.fixed == self.example
        assert entry.problems == [
            'PR link is not matching PR number 1949: "https://github.com/qubetics/qubetics/pull/1948"',
            'PR description should end with a dot: "Add `ClaimRewards` custom transaction"',
        ]

    def test_malformed_entry(self):
        malformed_example = (
            "- (distribution-precompile) [#194tps://github.com/qubetics/qubetics/pull/1"
        )
...
class TestCheckLink:
    example = "https://github.com/qubetics/qubetics/pull/1949"
    ...
    def test_wrong_base_url(self):
        fixed, problems = check_link("https://github.com/evmds/qubetics/pull/1949", 1949)
        assert fixed == self.example
        assert problems == [
            'PR link should point to qubetics repository: "https://github.com/evmds/qubetics/pull/1949"'
        ]

    def test_wrong_pr_number(self):
        fixed, problems = check_link("https://github.com/qubetics/qubetics/pull/1948", 1949)
        assert fixed == self.example
        assert problems == [
            'PR link is not matching PR number 1949: "https://github.com/qubetics/qubetics/pull/1948"'
        ]
