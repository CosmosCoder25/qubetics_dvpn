...
func NewConvertERC20Cmd() *cobra.Command {
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			contract := args[0]
			if err := qubeticstypes.ValidateAddress(contract); err != nil {
...
func NewConvertCoinCmd() *cobra.Command {
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if len(args) == 2 {
				receiver = args[1]
				if err := qubeticstypes.ValidateAddress(receiver); err != nil {
...
func NewRegisterERC20ProposalCmd() *cobra.Command {
	...
	cmd.Flags().String(cli.FlagDeposit, "1fut", "deposit of proposal")
