...
import (
	...
	"github.com/qubetics/qubetics-blockchain/v2/x/erc20/keeper"
...
func (suite *KeeperTestSuite) TestOnRecvPacket() {
	...
	ethsecpAddrQubetics := sdk.AccAddress(ethPk.PubKey().Address()).String()
	ethsecpAddrCosmos := sdk.MustBech32ifyAddressBytes(sdk.Bech32MainPrefix, ethsecpAddr)

	// Setup Cosmos <=> Qubetics IBC relayer
	sourceChannel := "channel-292"
	qubeticsChannel := "channel-3"
	path := fmt.Sprintf("%s/%s", transfertypes.PortID, qubeticsChannel)
	...
	testCases := []struct {
		...
		},
		{
			...
			malleate: func() {
				transfer := transfertypes.NewFungibleTokenPacketData(registeredDenom, "100", ethsecpAddrQubetics, ethsecpAddrCosmos, "")
				bz := transfertypes.ModuleCdc.MustMarshalJSON(&transfer)
				packet = channeltypes.NewPacket(bz, 1, transfertypes.PortID, sourceChannel, transfertypes.PortID, qubeticsChannel, timeoutHeight, 0)
			},
			receiver:      secpAddr,
			disableERC20:  true,
			ackSuccess:    true,
		...
		},
		{
			...
			malleate: func() {
				transfer := transfertypes.NewFungibleTokenPacketData(registeredDenom, "100", "qubetics", ethsecpAddrCosmos, "")
				bz := transfertypes.ModuleCdc.MustMarshalJSON(&transfer)
				packet = channeltypes.NewPacket(bz, 100, transfertypes.PortID, sourceChannel, transfertypes.PortID, qubeticsChannel, timeoutHeight, 0)
			},
			receiver:      secpAddr,
			ackSuccess:    false,
			checkBalances: false,
		...
		},
		{
			...
			malleate: func() {
				transfer := transfertypes.NewFungibleTokenPacketData(registeredDenom, "100", ethsecpAddrQubetics, "badbadhf0468jjpe6m6vx38s97z2qqe8ldu0njdyf625", "")
				bz := transfertypes.ModuleCdc.MustMarshalJSON(&transfer)
				packet = channeltypes.NewPacket(bz, 100, transfertypes.PortID, sourceChannel, transfertypes.PortID, qubeticsChannel, timeoutHeight, 0)
			},
			receiver:      secpAddr,
		...
		},
		{
			...
			malleate: func() {
				...
				transfer := transfertypes.NewFungibleTokenPacketData(prefixedDenom, "100", secpAddrCosmos, ethsecpAddrQubetics, "")
				bz := transfertypes.ModuleCdc.MustMarshalJSON(&transfer)
				packet = channeltypes.NewPacket(bz, 1, transfertypes.PortID, sourceChannel, transfertypes.PortID, qubeticsChannel, timeoutHeight, 0)
			},
			ackSuccess:    true,
		...
		},
		{
			...
			malleate: func() {
				transfer := transfertypes.NewFungibleTokenPacketData(erc20Denom, "100", secpAddrCosmos, ethsecpAddrQubetics, "")
				bz := transfertypes.ModuleCdc.MustMarshalJSON(&transfer)
				packet = channeltypes.NewPacket(bz, 1, transfertypes.PortID, sourceChannel, transfertypes.PortID, qubeticsChannel, timeoutHeight, 0)
			},
			ackSuccess:    true,
		...
		},
		{
			...
			malleate: func() {
				...
				otherSecpAddrQubetics := sdk.AccAddress(pk1.PubKey().Address()).String()
				transfer := transfertypes.NewFungibleTokenPacketData(prefixedDenom, "500", otherSecpAddrQubetics, ethsecpAddrQubetics, "")
				bz := transfertypes.ModuleCdc.MustMarshalJSON(&transfer)
				packet = channeltypes.NewPacket(bz, 1, transfertypes.PortID, sourceChannel, transfertypes.PortID, qubeticsChannel, timeoutHeight, 0)
			},
			ackSuccess: true,
	...
	for _, tc := range testCases {
		suite.Run(fmt.Sprintf("Case %s", tc.name), func() {
			...
			suite.app.IBCKeeper.ChannelKeeper.SetChannel(suite.ctx, transfertypes.PortID, qubeticsChannel, channel)

			// Set Next Sequence Send
			suite.app.IBCKeeper.ChannelKeeper.SetNextSequenceSend(suite.ctx, transfertypes.PortID, qubeticsChannel, 1)
			...
			// Fund receiver account with QUBETICS, ERC20 coins and IBC vouchers
...
func (suite *KeeperTestSuite) TestConvertCoinToERC20FromPacket() {
	senderAddr := "qubetics1gzsvk8rruqn2sx64acfsskrwy8hvrmaf6d7kvs"
	...
	testCases := []struct {
	...
	}{
		{
			...
			malleate: func() transfertypes.FungibleTokenPacketData {
				return transfertypes.NewFungibleTokenPacketData("tics", "10", "", "", "")
			},
			expPass: false,
		},
		{
			...
			malleate: func() transfertypes.FungibleTokenPacketData {
				return transfertypes.NewFungibleTokenPacketData("tics", "10", senderAddr, "", "")
			},
			expPass: true,
		},
		{
