...
import (
	...
	ibctesting "github.com/qubetics/qubetics-blockchain/v2/ibc/testing"
	...
	"github.com/qubetics/qubetics-blockchain/v2/x/erc20/keeper/testdata"
...
func (suite *KeeperTestSuite) SetupIBCTest() {
	...
	suite.QubeticsChain = suite.coordinator.GetChain(ibcgotesting.GetChainID(1))
	suite.IBCOsmosisChain = suite.coordinator.GetChain(ibcgotesting.GetChainID(2))
	suite.IBCCosmosChain = suite.coordinator.GetChain(ibcgotesting.GetChainID(3))
	suite.coordinator.CommitNBlocks(suite.QubeticsChain, 2)
	suite.coordinator.CommitNBlocks(suite.IBCOsmosisChain, 2)
	suite.coordinator.CommitNBlocks(suite.IBCCosmosChain, 2)

	s.app = suite.QubeticsChain.App.(*app.Qubetics)
	evmParams := s.app.EvmKeeper.GetParams(s.QubeticsChain.GetContext())
	evmParams.EvmDenom = utils.BaseDenom
	err := s.app.EvmKeeper.SetParams(s.QubeticsChain.GetContext(), evmParams)
	suite.Require().NoError(err)

	// s.app.FeeMarketKeeper.SetBaseFee(s.QubeticsChain.GetContext(), big.NewInt(1))

	// Set block proposer once, so its carried over on the ibc-go-testing suite
	validators := s.app.StakingKeeper.GetValidators(suite.QubeticsChain.GetContext(), 2)
	cons, err := validators[0].GetConsAddr()
	suite.Require().NoError(err)
	suite.QubeticsChain.CurrentHeader.ProposerAddress = cons.Bytes()

	err = s.app.StakingKeeper.SetValidatorByConsAddr(suite.QubeticsChain.GetContext(), validators[0])
	suite.Require().NoError(err)

	_, err = s.app.EvmKeeper.GetCoinbaseAddress(suite.QubeticsChain.GetContext(), sdk.ConsAddress(suite.QubeticsChain.CurrentHeader.ProposerAddress))
	suite.Require().NoError(err)
	// Mint coins locked on the qubetics account generated with secp.
	amt, ok := sdkmath.NewIntFromString("1000000000000000000000")
	suite.Require().True(ok)
	coinQubetics := sdk.NewCoin(utils.BaseDenom, amt)
	coins := sdk.NewCoins(coinQubetics)
	err = s.app.BankKeeper.MintCoins(suite.QubeticsChain.GetContext(), minttypes.ModuleName, coins)
	suite.Require().NoError(err)
	err = s.app.BankKeeper.SendCoinsFromModuleToAccount(suite.QubeticsChain.GetContext(), minttypes.ModuleName, suite.QubeticsChain.SenderAccount.GetAddress(), coins)
	suite.Require().NoError(err)
	...
	err = s.app.BankKeeper.MintCoins(s.QubeticsChain.GetContext(), types.ModuleName, coins)
	s.Require().NoError(err)
	coins = sdk.NewCoins(sdk.NewCoin(teststypes.UatomIbcdenom, sdkmath.NewInt(100)))
	err = s.app.BankKeeper.MintCoins(s.QubeticsChain.GetContext(), types.ModuleName, coins)
	s.Require().NoError(err)

	// Mint coins on the osmosis side which we'll use to unlock our tics
	...
	// Mint coins on the cosmos side which we'll use to unlock our tics
	...
	err = s.app.Erc20Keeper.SetParams(suite.QubeticsChain.GetContext(), params)
	suite.Require().NoError(err)

	suite.pathOsmosisQubetics = ibctesting.NewTransferPath(suite.IBCOsmosisChain, suite.QubeticsChain) // clientID, connectionID, channelID empty
	suite.pathCosmosQubetics = ibctesting.NewTransferPath(suite.IBCCosmosChain, suite.QubeticsChain)
	suite.pathOsmosisCosmos = ibctesting.NewTransferPath(suite.IBCCosmosChain, suite.IBCOsmosisChain)
	ibctesting.SetupPath(suite.coordinator, suite.pathOsmosisQubetics) // clientID, connectionID, channelID filled
	ibctesting.SetupPath(suite.coordinator, suite.pathCosmosQubetics)
	ibctesting.SetupPath(suite.coordinator, suite.pathOsmosisCosmos)
	suite.Require().Equal("07-tendermint-0", suite.pathOsmosisQubetics.EndpointA.ClientID)
	suite.Require().Equal("connection-0", suite.pathOsmosisQubetics.EndpointA.ConnectionID)
	suite.Require().Equal("channel-0", suite.pathOsmosisQubetics.EndpointA.ChannelID)

	coinQubetics = sdk.NewCoin(utils.BaseDenom, sdkmath.NewInt(1000000000000000000))
	coins = sdk.NewCoins(coinQubetics)
	err = s.app.BankKeeper.MintCoins(suite.QubeticsChain.GetContext(), types.ModuleName, coins)
	suite.Require().NoError(err)
	err = s.app.BankKeeper.SendCoinsFromModuleToModule(suite.QubeticsChain.GetContext(), types.ModuleName, authtypes.FeeCollectorName, coins)
	suite.Require().NoError(err)
}
...
// to the Qubetics chain (used on IBC tests)
func (suite *KeeperTestSuite) DeployContractToChain(name, symbol string, decimals uint8) (common.Address, error) {
	return testutil.DeployContract(
		s.QubeticsChain.GetContext(),
		s.QubeticsChain.App.(*app.Qubetics),
		suite.QubeticsChain.SenderPrivKey,
		suite.queryClientEvm,
		contracts.ERC20MinterBurnerDecimalsContract,
		name, symbol, decimals,
	)
...
// Send back coins (from path endpoint B to A). In case of IBC coins need to provide ibcCoinMetadata (<port>/<channel>/<denom>, e.g.: "transfer/channel-0/tics") as input parameter.
