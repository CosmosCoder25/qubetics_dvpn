	...
	testCases := []struct {
		...
		},
		{
			...
			map[string]func(*JumpTable){
				"qubetics_0": func(jt *JumpTable) {},
			},
			true,
			"",
			func() {
				eips := GetActivatorsEipNames()
				require.ElementsMatch(t, append(eips_snapshot, "qubetics_0"), eips, "expected eips number to be equal")
			},
		},
		{
			"success - multiple new activators",
			map[string]func(*JumpTable){
				"qubetics_1": func(jt *JumpTable) {},
				"qubetics_2": func(jt *JumpTable) {},
			},
			true,
			"",
			func() {
				...
				require.ElementsMatch(t, append(eips_snapshot, "qubetics_0", "qubetics_1", "qubetics_2"), eips, "expected eips number to be equal")
			},
		},
		{
