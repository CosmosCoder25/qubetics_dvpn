...
import (
	...
	"github.com/qubetics/qubetics-blockchain/v2/x/evm/core/logger"
	"github.com/qubetics/qubetics-blockchain/v2/x/evm/core/tracers"
...
func (k Keeper) Account(c context.Context, req *types.QueryAccountRequest) (*types.QueryAccountResponse, error) {
	...
	if err := qubeticstypes.ValidateAddress(req.Address); err != nil {
...
func (k Keeper) EstimateGasInternal(c context.Context, req *types.EthCallRequest, fromType types.CallType) (*types.EstimateGasResponse, error) {
	...
	executable := func(gas uint64) (vmError bool, rsp *types.MsgEthereumTxResponse, err error) {
		...
		if fromType == types.RPC {
			...
			gasMeter := qubeticstypes.NewInfiniteGasMeterWithLimit(msg.Gas())
...
func (k Keeper) TraceTx(c context.Context, req *types.QueryTraceTxRequest) (*types.QueryTraceTxResponse, error) {
	...
	for i, tx := range req.Predecessors {
		...
		ctx = evmante.BuildEvmExecutionCtx(ctx).
			WithGasMeter(qubeticstypes.NewInfiniteGasMeterWithLimit(msg.Gas()))
...
func getChainID(ctx sdk.Context, chainID int64) (*big.Int, error) {
	if chainID == 0 {
		return qubeticstypes.ParseChainID(ctx.ChainID())
	}
