...
func (k Keeper) CreateClawbackVestingAccount(
	...
	ethAcc, ok := acc.(*qubeticstypes.EthAccount)
	if !ok {
		return nil, errorsmod.Wrapf(errortypes.ErrInvalidRequest,
...
func (k Keeper) ConvertVestingAccount(
	...
	ethAccount := qubeticstypes.ProtoAccount().(*qubeticstypes.EthAccount)
...
// the destination address. Then, it updates the lockup schedule, removes qubetics
...
func (k Keeper) transferClawback(
	...
	// Compute clawback amount, unlock unvested tokens and remove qubetics vesting events
