...
const DefaultFeeAmt = int64(150_000_000_000_000_000) // 0.15 F

var globalStartTime = time.Date(2020, 1, 2, 0, 0, 0, 0, time.UTC)

// NewCoordinator initializes Coordinator with N EVM TestChain's (Qubetics apps) and M Cosmos chains (Simulation Apps)
...
func SendMsgs(chain *ibctesting.TestChain, feeAmt int64, msgs ...sdk.Msg) (*sdk.Result, error) {
	...
	if qubeticsChain, ok := chain.App.(*app.Qubetics); ok {
		bondDenom = qubeticsChain.StakingKeeper.BondDenom(chain.GetContext())
	} else {
