...
function checkTestEnv () {
  const argv = yargs(hideBin(process.argv))
    ...
    .example('$0 --network qubetics', 'run all tests using qubetics network')
    .example(
      '$0 --network qubetics --allowTests=test1,test2',
      'run only test1 and test2 using qubetics network'
    )
    .help('h')
    .alias('h', 'help')
    .describe('network', 'set which network to use: ganache|qubetics')
    .describe(
      'batch',
    ...
    .boolean('verbose-log')
    .describe('verbose-log', 'print qubeticsd output, default false').argv
  ...
  if (!argv.network) {
    ...
    if (argv.network !== 'qubetics' && argv.network !== 'ganache') {
      panic('network is invalid. Must be ganache or qubetics')
    } else {
...
function loadTests (runConfig) {
  ...
  fs.readdirSync(path.join(__dirname, 'suites')).forEach((dirname) => {
    ...
    try {
      ...
      const needScripts = ['test-ganache', 'test-qubetics']
...
function performTestSuite ({ testName, network }) {
  const cmd = network === 'ganache' ? 'test-ganache' : 'test-qubetics'
...
function setupNetwork ({ runConfig, timeout }) {
  if (runConfig.network !== 'qubetics') {
    // no need to start ganache. Truffle will start it
    return
  }

  // Spawn the qubetics process

  const spawnPromise = new Promise((resolve, reject) => {
    const serverStartedLog = 'Starting JSON-RPC server'
    const serverStartedMsg = 'qubeticsd started'

    const qubeticsdProc = spawn('../e2e/init-node.sh', {
      cwd: __dirname,
      stdio: ['ignore', 'pipe', 'pipe']
    })

    logger.info(`Starting qubeticsd process... timeout: ${timeout}ms`)
    if (runConfig.verboseLog) {
      qubeticsdProc.stdout.pipe(process.stdout)
    }

    qubeticsdProc.stdout.on('data', (d) => {
      ...
      if (oLine.indexOf(serverStartedLog) !== -1) {
        ...
        resolve(qubeticsdProc)
      }
    })

    qubeticsdProc.stderr.on('data', (d) => {
      const oLine = d.toString()
      if (runConfig.verboseLog) {
        process.stdout.write(oLine)
      }
  ...
  const timeoutPromise = new Promise((resolve, reject) => {
    setTimeout(() => reject(new Error('Start qubeticsd timeout!')), timeout)
  })
