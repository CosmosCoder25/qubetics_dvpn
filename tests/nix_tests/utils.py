...
QUBETICS_ADDRESS_PREFIX = "qubetics"
DEFAULT_DENOM = "tics"
WQUBETICS_ADDRESS = Web3.toChecksumAddress("0xD4949664cD82660AaE99bEdc034a0deA8A0bd517")
TEST_CONTRACTS = {
    ...
    "InterchainSender": "qubetics/testutil/contracts/InterchainSender.sol",
    "InterchainSenderCaller": "qubetics/testutil/contracts/InterchainSenderCaller.sol",
    "ICS20I": "qubetics/ics20/ICS20I.sol",
    "DistributionI": "qubetics/distribution/DistributionI.sol",
    "DistributionCaller": "qubetics/testutil/contracts/DistributionCaller.sol",
    "StakingI": "qubetics/staking/StakingI.sol",
    "StakingCaller": "qubetics/staking/testdata/StakingCaller.sol",
    "IStrideOutpost": "qubetics/outposts/stride/IStrideOutpost.sol",
    "IOsmosisOutpost": "qubetics/outposts/osmosis/IOsmosisOutpost.sol",
    "IERC20": "qubetics/erc20/IERC20.sol",
}

OSMOSIS_POOLS = {
    "Qubetics_Osmo": Path(__file__).parent / "osmosis/qubeticsOsmosisPool.json",
}
...
def eth_to_bech32(addr, prefix=QUBETICS_ADDRESS_PREFIX):
...
def update_node_cmd(path, cmd, i):
    ...
    for section in ini.sections():
        if section == f"program:qubetics_9000-1-node{i}":
            ini[section].update(
                {
...
def update_qubeticsd_and_setup_stride(modified_bin):
    def inner(path, base_port, config):
        update_qubetics_bin(modified_bin)(path, base_port, config)
        setup_stride()(path, base_port, config)

    return inner


def update_qubetics_bin(modified_bin, nodes=[0, 1]):
    """
    updates the qubetics binary with a patched binary.
...
def debug_trace_tx(qubetics, tx_hash: str):
    url = f"http://127.0.0.1:{ports.evmrpc_port(qubetics.base_port(0))}"
    params = {
