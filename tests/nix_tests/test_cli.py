...
def test_block_cmd(qubetics_cluster):
    """
    - start 2 qubetics nodes
    - wait for a certain height
    - stop the node1
    - use the 'block' cli cmd
    - restart qubetics node1
    """

    # wait for specific height
    node1 = qubetics_cluster.cosmos_cli(1)
    ...
    # start node1 again
    supervisorctl(
        qubetics_cluster.base_dir / "../tasks.ini", "start", "qubetics_9000-1-node1"
    )
