...
def custom_qubetics(tmp_path_factory):
    ...
    yield from setup_qubetics(path, 26700, long_timeout_commit=True)


@pytest.fixture(scope="module")
def custom_qubetics_rocksdb(tmp_path_factory):
    path = tmp_path_factory.mktemp("account-rocksdb")
    yield from setup_qubetics_rocksdb(
        path,
        26777,
    )
...
def test_query_qubetics_blk(cluster):
    w3: Web3 = cluster.w3
    acc = derive_new_account(2).address
    current = w3.eth.block_number
    qubetics = current + 1000
    with pytest.raises(ValueError) as exc:
        w3.eth.get_transaction_count(acc, hex(qubetics))
    print(acc, str(exc))
