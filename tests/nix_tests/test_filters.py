...
from .network import setup_custom_qubetics, setup_qubetics
from .utils import (
    ADDRS,
    CONTRACTS,
    deploy_contract,
...
def custom_qubetics(tmp_path_factory):
    ...
    yield from setup_qubetics(path, 26200)


@pytest.fixture(scope="module")
def qubetics_indexer(tmp_path_factory):
...
@pytest.fixture(
    scope="module",
    params=["qubetics", "geth", "qubetics-ws", "enable-indexer", "qubetics-rocksdb"],
)
def cluster(request, custom_qubetics, qubetics_indexer, qubetics_rocksdb, geth):
    """
    run on both qubetics and geth
    """
    ...
    if provider == "qubetics":
        ...
        qubetics_ws = custom_qubetics.copy()
        qubetics_ws.use_websocket()
        yield qubetics_ws
    elif provider == "enable-indexer":
        yield qubetics_indexer
