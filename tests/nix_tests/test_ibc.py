...
def test_ibc_transfer_with_hermes(ibc):
    ...
    dst_addr = ibc.chains["qubetics"].cosmos_cli().address("signer2")
    old_dst_balance = get_balance(ibc.chains["qubetics"], dst_addr, dst_denom)
    new_dst_balance = 0

    def check_balance_change():
        nonlocal new_dst_balance
        new_dst_balance = get_balance(ibc.chains["qubetics"], dst_addr, dst_denom)
    ...
    fee = int(events["tx"]["fee"].removesuffix("tics"))
...
def test_qubetics_ibc_transfer(ibc):
    """
...
def test_qubetics_ibc_transfer_acknowledgement_error(ibc):
    """
    test sending tics from qubetics to crypto-org-chain using cli
