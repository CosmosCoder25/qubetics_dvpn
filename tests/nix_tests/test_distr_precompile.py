...
def test_fund_community_pool(qubetics_cluster, name, deposit_amt, args, err_contains):
    """
    ...
    if deposit_amt is not None:
        ...
        deposit_receipt = send_transaction(
            qubetics_cluster.w3, deposit_tx, KEYS["signer1"]
        )
        assert deposit_receipt.status == 1, f"Failed: {name}"

        def check_contract_balance():
            new_contract_balance = qubetics_cluster.w3.eth.get_balance(
    ...
    signer1_prev_balance = get_balance(
        qubetics_cluster, qubetics_cluster.cosmos_cli().address("signer1"), denom
    )
    signer2_prev_balance = get_balance(
        qubetics_cluster, qubetics_cluster.cosmos_cli().address("signer2"), denom
    )

    community_prev_balance = qubetics_cluster.cosmos_cli().distribution_community()
    ...
    receipt = send_transaction(qubetics_cluster.w3, send_tx, KEYS["signer1"])

    if err_contains is not None:
        assert receipt.status == 0
        trace = debug_trace_tx(qubetics_cluster, receipt.transactionHash.hex())
    ...
    final_contract_balance = qubetics_cluster.w3.eth.get_balance(eth_contract.address)
    ...
    community_final_balance = qubetics_cluster.cosmos_cli().distribution_community()
    assert (
