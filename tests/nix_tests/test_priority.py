...
def custom_qubetics(tmp_path_factory):
    ...
    yield from setup_qubetics(path, 26800, long_timeout_commit=True)
...
def custom_qubetics_rocksdb(tmp_path_factory):
    ...
    # run with long timeout commit to ensure all
    # txs are included in the same block
    yield from setup_qubetics_rocksdb(path, 26810, long_timeout_commit=True)
...
def test_priority(qubetics_cluster):
    """
...
def test_native_tx_priority(qubetics_cluster):
    ...
    test_cases = [
        {
            ...
            "amount": "1000tics",
            "gas_prices": f"{base_fee + PRIORITY_REDUCTION * 600000}tics",
        ...
        },
        {
            ...
            "amount": "1000tics",
            "gas_prices": f"{base_fee + PRIORITY_REDUCTION * 400000}tics",
    ...
    for tc in test_cases:
        ...
        gas_price = int(tc["gas_prices"].removesuffix("tics"))
