...
def test_ibc_transfer(ibc):
    ...
    cli = ibc.chains["qubetics"].cosmos_cli()
    src_addr = cli.address("signer2")
    src_denom = "tics"

    old_src_balance = get_balance(ibc.chains["qubetics"], src_addr, src_denom)
    old_dst_balance = get_balance(ibc.chains["chainmain"], dst_addr, QUBETICS_IBC_DENOM)
    ...
    gas_estimation = ibc.chains["qubetics"].w3.eth.estimate_gas(tx)
    receipt = send_transaction(ibc.chains["qubetics"].w3, tx, KEYS["signer2"])
    ...
    fee = receipt.gasUsed * qubetics_gas_price
...
def test_ibc_transfer_invalid_packet(ibc):
    ...
    try:
        pc.functions.transfer(
            ...
            0,
            "",
        ).transact({"from": ADDRS["signer2"], "gasPrice": qubetics_gas_price})
...
def test_staking(ibc):
    ...
    old_src_balance = get_balance(qubetics, del_addr, src_denom)
    ...
    tx = pc.functions.delegate(ADDRS["signer2"], validator_addr, amt).build_transaction(
        {"from": ADDRS["signer2"], "gasPrice": qubetics_gas_price}
    )
    gas_estimation = qubetics.w3.eth.estimate_gas(tx)
    ...
    new_src_balance = get_balance(qubetics, del_addr, src_denom)
    assert old_src_balance - amt - fee == new_src_balance
...
def test_staking_via_sc(ibc):
    ...
    approve_tx = contract.functions.testApprove(
        ...
        {"from": ADDRS["signer1"], "gasPrice": qubetics_gas_price, "gas": 60000}
    )

    gas_estimation = qubetics.w3.eth.estimate_gas(approve_tx)
    ...
    delegate_tx = contract.functions.testDelegate(
        ...
        {"from": ADDRS["signer1"], "gasPrice": qubetics_gas_price, "gas": 180000}
    )
    gas_estimation = qubetics.w3.eth.estimate_gas(delegate_tx)
