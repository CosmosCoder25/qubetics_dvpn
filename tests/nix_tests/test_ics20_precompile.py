...
from .ibc_utils import QUBETICS_IBC_DENOM, assert_ready, get_balance, prepare_network
from .network import Qubetics
from .utils import (
    ACCOUNTS,
    ADDRS,
    CONTRACTS,
...
def test_ibc_transfer_from_contract(ibc):
    ...
    pc = get_precompile_contract(ibc.chains["qubetics"].w3, "ICS20I")
    qubetics_gas_price = ibc.chains["qubetics"].w3.eth.gas_price

    src_adr = ibc.chains["qubetics"].cosmos_cli().address("signer2")
    ...
    tx_receipt = send_transaction(ibc.chains["qubetics"].w3, approve_tx, KEYS["signer2"])
    assert tx_receipt.status == 1
    ...
    src_amount_qubetics_prev = get_balance(ibc.chains["qubetics"], src_adr, src_denom)
    ...
    deposit_receipt = send_transaction(
        ibc.chains["qubetics"].w3, deposit_tx, KEYS["signer2"]
    )
    assert deposit_receipt.status == 1
    fees = deposit_receipt.gasUsed * qubetics_gas_price
    ...
    receipt = send_transaction(ibc.chains["qubetics"].w3, send_tx, KEYS["signer2"])
    assert receipt.status == 1
    fees += receipt.gasUsed * qubetics_gas_price
    ...
    src_amount_qubetics = get_balance(ibc.chains["qubetics"], src_adr, src_denom)
    assert src_amount_qubetics == src_amount_qubetics_prev - amt - fees
...
def test_ibc_transfer_from_eoa_through_contract(ibc):
    ...
    src_starting_balance = get_balance(ibc.chains["qubetics"], src_adr, "tics")
    ...
    send_tx = eth_contract.functions.transferFromEOA(
        ...
        {"from": ADDRS["signer2"], "gasPrice": qubetics_gas_price, "gas": gas_limit}
    )
    receipt = send_transaction(ibc.chains["qubetics"].w3, send_tx, KEYS["signer2"])
    assert receipt.status == 1
    fees = receipt.gasUsed * qubetics_gas_price
    ...
    src_final_amount_qubetics = get_balance(ibc.chains["qubetics"], src_adr, src_denom)
    assert src_final_amount_qubetics == src_starting_balance - amt - fees
...
def test_ibc_transfer_from_eoa_with_internal_transfer(
    ...
    src_denom = "tics"
    gas_limit = 200_000
    qubetics_gas_price = w3.eth.gas_price

    dst_addr = ibc.chains["chainmain"].cosmos_cli().address("signer2")
    # address that will escrow the tics tokens when transferring via IBC
    escrow_bech32 = qubetics.cosmos_cli().escrow_address(channel)
    ...
    eth_contract = setup_interchain_sender_contract(
        qubetics, ACCOUNTS["signer2"], amt, initial_contract_balance
    )

    # get starting balances to check after the tx
    src_bech32 = qubetics.cosmos_cli().address("signer2")
    contract_bech32 = eth_to_bech32(eth_contract.address)
    other_addr_bech32 = eth_to_bech32(other_addr)

    src_starting_balance = get_balance(ibc.chains["qubetics"], src_bech32, "tics")
    dest_starting_balance = get_balance(
        ibc.chains["chainmain"], dst_addr, QUBETICS_IBC_DENOM
    )
    other_addr_initial_balance = get_balance(
        ibc.chains["qubetics"], other_addr_bech32, src_denom
    )
    escrow_initial_balance = get_balance(ibc.chains["qubetics"], escrow_bech32, src_denom)
    ...
    if other_addr == src_addr:
        ...
        other_addr_final_balance = get_balance(
            ibc.chains["qubetics"], escrow_bech32, src_denom
        )
    ...
    src_final_amount_qubetics = get_balance(ibc.chains["qubetics"], src_bech32, src_denom)
    assert src_final_amount_qubetics == exp_src_final_bal, f"Failed {name}"

    # Check contracts final state (balance & counter)
    contract_final_balance = get_balance(
        ibc.chains["qubetics"], contract_bech32, src_denom
    )
    assert (
    ...
    escrow_final_balance = get_balance(ibc.chains["qubetics"], escrow_bech32, src_denom)
...
def test_ibc_multi_transfer_from_eoa_with_internal_transfer(
    ...
    dst_addr = ibc.chains["chainmain"].cosmos_cli().address("signer2")
    escrow_bech32 = qubetics.cosmos_cli().escrow_address("channel-0")

    # Deployment of contracts and initial checks
    initial_contract_balance = 100
    eth_contract = setup_interchain_sender_contract(
        qubetics, ACCOUNTS["signer2"], ibc_transfer_amt, initial_contract_balance
    )

    # send some funds (100 tics) to the contract to perform
    # internal transfer within the tx
    src_bech32 = eth_to_bech32(src_addr)
    contract_bech32 = eth_to_bech32(eth_contract.address)

    # get starting balances to check after the tx
    src_starting_balance = get_balance(qubetics, src_bech32, "tics")
    dest_starting_balance = get_balance(
        ibc.chains["chainmain"], dst_addr, QUBETICS_IBC_DENOM
    )
    escrow_initial_balance = get_balance(qubetics, escrow_bech32, src_denom)
    ...
    if err_contains is not None:
        ...
        trace = debug_trace_tx(ibc.chains["qubetics"], receipt.transactionHash.hex())
    ...
    assert receipt.status == 1, debug_trace_tx(
        ibc.chains["qubetics"], receipt.transactionHash.hex()
    )
    fees = receipt.gasUsed * qubetics_gas_price
...
def test_multi_ibc_transfers_with_revert(ibc):
    ...
    dst_addr = ibc.chains["chainmain"].cosmos_cli().address("signer2")
    escrow_bech32 = ibc.chains["qubetics"].cosmos_cli().escrow_address("channel-0")

    # Deployment of contracts and initial checks
    initial_contract_balance = 100
    interchain_sender_contract = setup_interchain_sender_contract(
        qubetics, ACCOUNTS["signer2"], ibc_transfer_amt * 2, initial_contract_balance
    )
    ...
    assert src_final_amount_qubetics == exp_src_final_bal

    # Check contracts final state (balance & counter)
    contract_final_balance = get_balance(
        ibc.chains["qubetics"], interchain_sender_contract_bech32, src_denom
    )
    assert (
        contract_final_balance
        == initial_contract_balance - exp_amt_tranferred_internally
...
def setup_interchain_sender_contract(
    qubetics, src_acc, transfer_amt, initial_contract_balance
):
    """
    ...
    src_denom = "tics"
    gas_limit = 200_000
    qubetics_gas_price = qubetics.w3.eth.gas_price
    # Deployment of contracts and initial checks
    eth_contract, tx_receipt = deploy_contract(qubetics.w3, CONTRACTS["InterchainSender"])
    assert tx_receipt.status == 1

    counter = eth_contract.functions.counter().call()
    assert counter == 0

    pc = get_precompile_contract(qubetics.w3, "ICS20I")
    # Approve the contract to spend the src_denom
    approve_tx = pc.functions.approve(
    ...
    tx_receipt = send_transaction(qubetics.w3, approve_tx, src_acc.key)
    assert tx_receipt.status == 1
    ...
    contract_bech32 = eth_to_bech32(eth_contract.address)
    fund_tx = qubetics.cosmos_cli().transfer(
        src_bech32,
        contract_bech32,
        f"{initial_contract_balance}tics",
        gas_prices=f"{qubetics_gas_price + 100000}tics",
        generate_only=True,
    )

    fund_tx = qubetics.cosmos_cli().sign_tx_json(fund_tx, src_bech32, max_priority_price=0)
    rsp = qubetics.cosmos_cli().broadcast_tx_json(fund_tx, broadcast_mode="sync")
    assert rsp["code"] == 0, rsp["raw_log"]
    txhash = rsp["txhash"]
    wait_for_new_blocks(qubetics.cosmos_cli(), 2)
    receipt = qubetics.cosmos_cli().tx_search_rpc(f"tx.hash='{txhash}'")[0]
