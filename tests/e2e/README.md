...
for running end-to-end tests on Qubetics.
It is used for testing chain upgrades,
as it allows for initializing multiple Qubetics chains with different versions.
...
# version(s) of initial qubetics node(s) that will be upgraded, tag e.g. 'v9.1.0'
# to use multiple upgrades separate the versions with a forward slash, e.g.
# 'v10.0.1/v11.0.0-rc1'
INITIAL_VERSION

# version of upgraded qubetics node that will replace the initial node, tag e.g.
# 'v10.0.0'
TARGET_VERSION

# mount point for the upgraded node container, to mount new node version to
# previous node state folder. By default this is './build/.qubeticsd:/root/.qubeticsd'
# More info at https://docs.docker.com/engine/reference/builder/#volume
MOUNT_PATH

# '--chain-id' qubetics cli parameter, used to start nodes with a specific
# chain-id and submit proposals
# By default this is 'qubetics_9000-1'
CHAIN_ID
```
...
1. Build a docker image for the qubetics target version
...
the test suite exports `/.qubeticsd` folder from the docker container
...
This package is decoupled from depending on the Qubetics codebase.
It initializes the chains for testing via Docker.  
As a result, the test suite may provide the
desired Qubetics version to Docker containers during the initialization.
This design allows for the opportunity of testing chain upgrades
by providing an older Qubetics version to the container,
...
to run different versions of qubetics containers,
...
will be pulled from [DockerHub](https://hub.docker.com/r/tharsishq/qubetics/tags).
If it is not specified,
the test suite retrieves the second-to-last upgrade version
from the local codebase (in the `qubetics/app/upgrades` folder)
...
```log
ok  	github.com/qubetics/qubetics/v9/tests/e2e	174.137s.
```
...
Error:  Received unexpected error:
        can't start qubetics node, container exit code: 2
        ...
        main.appCreator.newApp({{{0x3399b40, 0xc000ec1db8}, {0x33ac0f8, 0xc00...
            github.com/qubetics/qubetics/v10/cmd/qubeticsd/root.go:243 +0x2ca
        github.com/qubetics/ethermint/server.startInProcess(_, {{0x0, 0x0, 0x0},...
            github.com/qubetics/ethermint@v0.20.0-rc2/server/start.go:304 +0x9c5
        github.com/qubetics/ethermint/server.StartCmd.func2(0xc001620600?, {0xc0...
            github.com/qubetics/ethermint@v0.20.0-rc2/server/start.go:123 +0x1ec
        ...
        main.main()
            github.com/qubetics/qubetics/v10/cmd/qubeticsd/main.go:20 +0x45
...
```log
CONTAINER ID   IMAGE
9307f5485323   qubetics:local    <-- upgraded node
f41c97d6ca21   qubetics:v9.0.0   <-- initial node
```
