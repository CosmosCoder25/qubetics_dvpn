...
# Set client config
qubeticsd config keyring-backend "$KEYRING"
qubeticsd config chain-id "$CHAINID"

# Import keys from mnemonics
echo "$VAL_MNEMONIC" | qubeticsd keys add "$VAL_KEY" --recover --keyring-backend "$KEYRING" --algo "$KEYALGO"

# Store the validator address in a variable to use it later
node_address=$(qubeticsd keys show -a "$VAL_KEY")

echo "$USER1_MNEMONIC" | qubeticsd keys add "$USER1_KEY" --recover --keyring-backend "$KEYRING" --algo "$KEYALGO"
echo "$USER2_MNEMONIC" | qubeticsd keys add "$USER2_KEY" --recover --keyring-backend "$KEYRING" --algo "$KEYALGO"
echo "$USER3_MNEMONIC" | qubeticsd keys add "$USER3_KEY" --recover --keyring-backend "$KEYRING" --algo "$KEYALGO"
echo "$USER4_MNEMONIC" | qubeticsd keys add "$USER4_KEY" --recover --keyring-backend "$KEYRING" --algo "$KEYALGO"

# Set moniker and chain-id for Qubetics (Moniker can be anything, chain-id must be an integer)
qubeticsd init "$MONIKER" --chain-id "$CHAINID"
...
# 0xA61808Fe40fEb8B3433778BBC2ecECCAA47c8c47 || qubetics15cvq3ljql6utxseh0zau9m8ve2j8erz89m5wkz
jq -r --arg amount_to_claim "$amount_to_claim" '.app_state.bank.balances += [{"address":"qubetics15cvq3ljql6utxseh0zau9m8ve2j8erz89m5wkz","coins":[{"denom":"tics", "amount":$amount_to_claim}]}]' "$GENESIS" >"$TMP_GENESIS" && mv "$TMP_GENESIS" "$GENESIS"
...
qubeticsd add-genesis-account "$(qubeticsd keys show "$VAL_KEY" -a --keyring-backend "$KEYRING")" 100000000000000000000000000tics --keyring-backend "$KEYRING"
qubeticsd add-genesis-account "$(qubeticsd keys show "$USER1_KEY" -a --keyring-backend "$KEYRING")" 1000000000000000000000tics --keyring-backend "$KEYRING"
qubeticsd add-genesis-account "$(qubeticsd keys show "$USER2_KEY" -a --keyring-backend "$KEYRING")" 1000000000000000000000tics --keyring-backend "$KEYRING"
qubeticsd add-genesis-account "$(qubeticsd keys show "$USER3_KEY" -a --keyring-backend "$KEYRING")" 1000000000000000000000tics --keyring-backend "$KEYRING"
qubeticsd add-genesis-account "$(qubeticsd keys show "$USER4_KEY" -a --keyring-backend "$KEYRING")" 1000000000000000000000tics --keyring-backend "$KEYRING"
...
qubeticsd gentx "$VAL_KEY" 1000000000000000000000tics --gas-prices ${BASEFEE}tics --keyring-backend "$KEYRING" --chain-id "$CHAINID"
...
# Collect genesis tx
qubeticsd collect-gentxs

# Run this to ensure everything worked and that the genesis file is setup correctly
qubeticsd validate-genesis

# Start the node
qubeticsd start "$TRACE" \
  --log_level $LOGLEVEL \
