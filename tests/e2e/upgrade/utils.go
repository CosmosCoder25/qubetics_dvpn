...
// QubeticsVersions is a custom comparator for sorting semver version strings.
type QubeticsVersions []string

// Len is the number of stored versions.
func (v QubeticsVersions) Len() int { return len(v) }

// Swap swaps the elements with indexes i and j. It is needed to sort the slice.
func (v QubeticsVersions) Swap(i, j int) { v[i], v[j] = v[j], v[i] }

// Less compares semver versions strings properly
func (v QubeticsVersions) Less(i, j int) bool {
...
func CheckLegacyProposal(version string) bool {
	...
	cmp := QubeticsVersions([]string{version, "v10.0.0"})
...
func RetrieveUpgradesList(upgradesPath string) ([]string, error) {
	...
	sort.Sort(QubeticsVersions(versions))

	return versions, nil
}

// ExportState executes the  'docker cp' command to copy container .qubeticsd dir
...
func (m *Manager) ExportState(targetDir string) error {
	...
	cmd := exec.Command(
		"docker",
		"cp",
		fmt.Sprintf("%s:/root/.qubeticsd", m.ContainerID()),
		targetDir,
	)
	return cmd.Run()
}
