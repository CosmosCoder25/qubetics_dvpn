...
func (m *Manager) BuildImage(name, version, dockerFile, contextDir string, args map[string]string) error {
	...
	opts := docker.BuildImageOptions{
		...
		NoCache: true,
		// name with tag, e.g. qubetics:v9.0.0
	...
	err = m.pool.Retry(
		func() error {
			...
			if c.State.ExitCode != 0 {
				stdOut, stdErr, _ := m.GetLogs(resource.Container.ID)
				return fmt.Errorf(
					"can't start qubetics node, container exit code: %d\n\n[error stream]:\n\n%s\n\n[output stream]:\n\n%s",
					c.State.ExitCode,
					stdErr,
					stdOut,
				)
...
// WaitForHeight queries the Qubetics node every second until the node will reach the specified height.
...
// GetNodeHeight calls the Qubetics CLI in the current node container to get the current block height
func (m *Manager) GetNodeHeight(ctx context.Context) (int, error) {
	exec, err := m.CreateExec([]string{"qubeticsd", "q", "block"}, m.ContainerID())
	if err != nil {
	...
	if errBuff.String() != "" {
		return 0, fmt.Errorf("qubetics query error: %s", errBuff.String())
	}
	return h, nil
}

// GetNodeVersion calls the Qubetics CLI in the current node container to get the
// current node version
func (m *Manager) GetNodeVersion(ctx context.Context) (string, error) {
	exec, err := m.CreateExec([]string{"qubeticsd", "version"}, m.ContainerID())
	if err != nil {
	...
	if errBuff.String() != "" {
		return "", fmt.Errorf("qubetics version error: %s", errBuff.String())
	}
...
func (m *Manager) getTimeoutCommit(ctx context.Context) (*big.Int, error) {
	exec, err := m.CreateExec([]string{"grep", `\s*timeout_commit =`, "/root/.qubeticsd/config/config.toml"}, m.ContainerID())
	if err != nil {
	...
	if errBuff.String() != "" {
		return common.Big0, fmt.Errorf("qubetics version error: %s", errBuff.String())
	}
