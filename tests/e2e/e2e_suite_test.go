...
import (
	...
	"github.com/qubetics/qubetics-blockchain/v2/tests/e2e/upgrade"
...
const (
	...
	defaultManagerNetwork = "qubetics-local"
...
// runInitialNode builds a docker image capable of running an Qubetics node with the given version.
...
func (s *IntegrationTestSuite) runInitialNode(version upgrade.VersionConfig) {
	...
	s.Require().NoError(err, "can't build container with Qubetics version: %s", version.ImageTag)
	...
	s.Require().NoError(err, "can't run node with Qubetics version: %s", version)
...
// runNodeWithCurrentChanges builds a docker image using the current branch of the Qubetics repository.
...
func (s *IntegrationTestSuite) runNodeWithCurrentChanges() {
	...
	err = s.upgradeManager.RunNode(node)
	s.Require().NoError(err, "can't run node Qubetics using branch %s", branch)
}
...
func (s *IntegrationTestSuite) proposeUpgrade(name, target string) {
	...
	// if Qubetics is lower than v10.x.x no need to use the legacy proposal
	currentVersion, err := s.upgradeManager.GetNodeVersion(ctx)
	s.Require().NoError(err, "can't get current Qubetics version")
	...
	exec, err := s.upgradeManager.CreateSubmitProposalExec(
		...
		isLegacyProposal,
		"--fees=10000000000000000tics",
		"--gas=500000",
	)
	s.Require().NoErrorf(
		err,
		"can't create the proposal to upgrade Qubetics to %s at height %d with name %s",
	...
	s.Require().NoErrorf(
		err,
		"failed to submit proposal to upgrade Qubetics to %s at height %d\nstdout: %s,\nstderr: %s",
...
func (s *IntegrationTestSuite) voteForProposal(id int) {
	ctx, cancel := context.WithCancel(context.Background())
	defer cancel()
	exec, err := s.upgradeManager.CreateVoteProposalExec(s.upgradeParams.ChainID, id, "--fees=10000000000000000tics", "--gas=500000")
...
func (s *IntegrationTestSuite) upgrade(targetRepo, targetVersion string) {
	...
	// export node .qubeticsd to local build/
	...
	node.SetCmd([]string{"qubeticsd", "start", fmt.Sprintf("--chain-id=%s", s.upgradeParams.ChainID), fmt.Sprintf("--home=%s.qubeticsd", rootDir)})
	err = s.upgradeManager.RunNode(node)
