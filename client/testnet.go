...
import (
	...
	"github.com/qubetics/qubetics-blockchain/v2/testutil/network"
)
...
Example:
	qubeticsd testnet init-files --v 4 --output-dir ./.testnets --starting-ip-address 192.168.10.2
	`,
...
Example:
	qubeticsd testnet --v 4 --output-dir ./.testnets
	`,
		RunE: func(cmd *cobra.Command, _ []string) error {
			args := startArgs{}
			args.outputDir, _ = cmd.Flags().GetString(flagOutputDir)
...
func initTestnetFiles(
	...
	for i := 0; i < args.numValidators; i++ {
		...
		accStakingTokens := sdk.TokensFromConsensusPower(5000, qubeticstypes.PowerReduction)
		coins := sdk.Coins{
			sdk.NewCoin(qubeticstypes.AttoQubetics, accStakingTokens),
		}
		...
		valTokens := sdk.TokensFromConsensusPower(100, qubeticstypes.PowerReduction)
		createValMsg, err := stakingtypes.NewMsgCreateValidator(
			sdk.ValAddress(addr),
			valPubKeys[i],
			sdk.NewCoin(qubeticstypes.AttoQubetics, valTokens),
		...
		customAppTemplate, customAppConfig := config.AppConfig(qubeticstypes.AttoQubetics)
	...
	if err := initGenFiles(clientCtx, mbm, args.chainID, qubeticstypes.AttoQubetics, genAccounts, genBalances, genFiles, args.numValidators); err != nil {
		return err
	}
